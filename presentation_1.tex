...
\date{June 2015} % Date, can be changed to a custom date
...
\For{$r \in R$}
\If{$P \cap K_r = \emptyset$} //keyword is not present
\State continue;
\EndIf
\item[]
\If{$P \cap \Delta \not = \emptyset$} //delta words are present
\State continue;
\EndIf
\If{$Unit(n) \not \in LegalUnits(r)$} //incompatible units?
\State continue;
\EndIf
...
%\begin{small}
\hline
%\textbf{Relation} &  \multicolumn{3}{c|}{\textbf{Numbertron}} &  \multicolumn{3}{|c|}{\textbf{NumberRule}} \\
%\hline
...
\frametitle{Summary}
\begin{tabularx}{\textwidth}{|b |b |b |}
 \textbf{Use of Unit Tagger} & Units are used to test compatibility of a relation and the number.& Units are used for training data creation and flattening to SI units.\\ \hline
 \textbf{Common Number Pruning} & N/A & Features included to capture type (whole, fraction), magnitude and frequency. \\ \hline
 \textbf{Modified Relations} & Handled by forming relation phrases: attaching keywords with words related via modifying dependencies, like \emph{urban} population.& Not handled in the model, can be handled at the time of extraction using a scheme similar to the one used by NumerRule. \\ \hline
